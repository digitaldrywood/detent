...
| `instruction_budget` | Estimates the repository AGENTS chain (root to workdir, override-file precedence, **32 KiB aggregate cap**), effective WORKFLOW prompt, and literal file paths (filenames with extensions, `Makefile`, or backtick-quoted paths) on read/follow/consult/load/review directive lines. Lists files and bytes; **warns above 24 KiB, fails above 48 KiB**. Missing evidence warns. |
...
| `model_policy` | Prints each effective level's model, effort, and configuration sources. Warns when **more than 10%** of project `codex_sessions` started in the last 24 hours have `issue.effort` above the current level default, or provenance cannot be compared. |
