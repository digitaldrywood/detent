...
before promotion or programmatic merge. Native queue admission waits for that
...
COMMENTED with no findings; an unanswered request remains pending. A disabled
initial-review requirement or expired review deadline does not waive the cycle
for promotion or programmatic merge.
