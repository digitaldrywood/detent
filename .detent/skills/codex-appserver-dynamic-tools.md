...
- Distinguish supplemental worker tools from operator-only tools with an explicit runner request field. Adding a tracker helper to an implementation turn must retain its configured coding sandbox, approval policy, and worker instructions; tool presence alone must not select board-operator policy. An explicit read-only request still takes precedence.
- Test the full backend request for ordinary workers, operator sessions, and read-only workers with supplemental tools. Assert dynamic tool registration alongside developer instructions, thread and turn sandbox policies, and approval policy, then verify that worker request construction forwards the mode.
