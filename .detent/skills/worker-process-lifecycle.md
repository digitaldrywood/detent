...
- When a process-group signal returns `EPERM`, inspect the group before classifying the result: accept it as exited only when no members or zombie-only members remain, and preserve the error when a live member remains or inspection fails.
- Reproduce ambiguous process-group signal races by injecting the signal and inspection boundaries; cover exited, zombie-only, live unauthorized, and inspection-failure results without relying on the host kernel to hit the race.
