...
func appendBlockedHandoffBlock(prompt string, opts PromptOptions) string {
	...
	return strings.TrimRight(prompt, " \t\r\n") + "\n\n## Blocked handoff\n\n" +
		...
		"Worker GitHub credentials and connector write-policy denials are instance conditions. Do not use ask_human_question, create a prerequisite issue, or request a manual pull request for them; report the exact failure so Detent can pause and probe the project. " +
		"For a missing human decision, finish independent implementation, stub-testable work, and authorized fallbacks first. Use ask_human_question to post one concise question on the assigned issue and persist the wait. Explain the concrete blocker, researched recommendation, practical alternatives, and exact decision needed. Never create a separate issue, human-owned milestone, label, or dependency edge for a clarification or approval, including through gh or tracker tools. " +
		"Read ordinary authorized replies in the original thread. Interpret only the permission actually expressed; a generic reply or migration does not authorize live sends, publishing, deployment, or destructive actions. Ask a focused follow-up when ambiguous. No special command, detent-human YAML, completion_evidence, or issue closure is required. Keep the issue lane and existing PR intact; record in_progress in the Workpad while Detent holds the internal question wait. Never acknowledge independent breaker parks. If the question tool is unavailable, report the missing durable question capability rather than creating a synthetic dependency. " +
		"The following dependency instructions apply only to genuine software dependencies, never human questions. " +
