...
func TestBuildPromptSecurityScope(t *testing.T) {
	...
	tests := []struct{ name, instruction string }{
		...
		{"requested behavior", "A product or business-rule decision the issue explicitly makes is never itself a finding at any severity"},
		{"requested defects", "This is not an exemption for security defects, including defects that arise from the requested behavior itself"},
		{"defect attribution", "Report such defects at their real severity and state plainly when they arise from the requested behavior"},
		{"requested authorization bypass", "if an issue explicitly requests removing an authorization check and the diff enables cross-tenant access, report a p1 authorization bypass finding and verdict fail"},
