...
func (w *recordingWaker) Available() bool { return true }
...
func TestConversationServiceWakesPendingWorkAfterRestart(t *testing.T) {
	...
	if got := coordinator.woken; len(got) != 1 || got[0] != unlinked.ID {
		t.Fatalf("coordinator woken = %v, want [%s]", got, unlinked.ID)
	}
