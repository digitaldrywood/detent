...
func (s *Service) registerHostedOrganizationRoutes(e *echo.Echo) {
	...
	e.POST(hostedOrganizationBase+"/invitations/accept", s.acceptHostedInvitation, session)
	e.POST(hostedOrganizationBase+"/switch", s.switchHostedOrganization, session)
	e.POST(hostedOrganizationBase+"/support/start", s.startHostedSupport, session)
	...
	e.GET(hostedOrganizationBase+"/usage", s.hostedUsageReport, session)
	e.GET(hostedOrganizationBase+"/plan", s.hostedPlanReport, session)
	e.GET(hostedOrganizationBase+"/billing", s.hostedBillingReportJSON, session)
	e.POST(hostedOrganizationBase+"/billing/checkout", s.hostedBillingCheckout, session)
	e.POST(hostedOrganizationBase+"/billing/portal", s.hostedBillingPortal, session)
}
...
// createHostedOrganizationJSON answers POST /api/v2/organizations for the
// reserved bootstrap subject creating this hub's first organization.
func (s *Service) createHostedOrganizationJSON(c echo.Context) error {
	if c.Request().Header.Get(echo.HeaderAuthorization) != "" {
		// Instance administration does not create hosted organizations.
	...
	if err := s.database.db.QueryRowContext(c.Request().Context(), "SELECT count(*) FROM hosted_members").Scan(&existingMembers); err != nil || existingMembers != 0 {
		return s.hostedError(c, http.StatusConflict, "This organization has already been created")
	}
	...
	}{appBootstrapOrganization{ID: s.config.Hosted.OrganizationID, Name: name, PublicURL: s.config.Hosted.PublicURL, Current: true}, "/auth/oidc/start"})

}

// acceptHostedInvitation answers POST /invitations/accept. The caller signs in
// again afterwards so the new membership is minted into the session.
func (s *Service) acceptHostedInvitation(c echo.Context) error {
	...
	if err := s.acceptHostedInvitationFor(c.Request().Context(), identity, request.Token); err != nil {
...
// switchHostedOrganization answers POST /switch with the sign-in entry point
...
func (s *Service) switchHostedOrganization(c echo.Context) error {
	...
	for _, destination := range s.config.Hosted.Directory {
		if destination.OrganizationID != request.Organization {
			continue
		}
		...
		for _, membership := range memberships {
			if membership.Status == "active" && membership.UserID == session.Identity.Subject && membership.OrganizationID == destination.WorkOSOrganizationID {
				return c.JSON(http.StatusOK, struct {
					Next string `json:"next"`
				}{destination.PublicURL + "/auth/oidc/start"})
			}
		}
	}
