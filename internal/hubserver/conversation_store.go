...
// Coordinator items (decisions section 9.1).

// coordinatorItemRecord associates a native issue that dispatches a
// coordinator turn with the conversation it answers. The conversation stays
// unlinked and private: this row, not conversations.work_item_id, is the
// association.
type coordinatorItemRecord struct {
	WorkItemID     string
	ConversationID string
	OrganizationID tracker.OrganizationID
	ProjectID      tracker.ProjectID
...
// openCoordinatorItem returns the conversation's open coordinator item. A
// conversation has at most one: the next message reuses it and the unbind
// closes it.
func (s *conversationStore) openCoordinatorItem(ctx context.Context, query nativeQueryer, conversationID string) (coordinatorItemRecord, bool, error) {
	var record coordinatorItemRecord
	var created string
	err := query.QueryRowContext(ctx, `SELECT work_item_id, conversation_id, organization_id, project_id, created_at
FROM coordinator_items WHERE conversation_id = ? AND closed_at IS NULL ORDER BY created_at, work_item_id LIMIT 1`, conversationID).
		Scan(&record.WorkItemID, &record.ConversationID, &record.OrganizationID, &record.ProjectID, &created)
	if errors.Is(err, sql.ErrNoRows) {
		return record, false, nil
	}
	if err != nil {
		return record, false, fmt.Errorf("read coordinator item: %w", err)
	}
	if record.CreatedAt, err = parseTimeValue(created); err != nil {
		return record, false, fmt.Errorf("decode coordinator item %s created_at: %w", record.WorkItemID, err)
	}
	return record, true, nil
}

func (s *conversationStore) createCoordinatorItem(ctx context.Context, tx *sql.Tx, record coordinatorItemRecord) error {
	if record.WorkItemID == "" || record.ConversationID == "" {
		return nativeInvalid("A coordinator item requires a work item and a conversation")
	}
	if _, err := tx.ExecContext(ctx, `INSERT INTO coordinator_items (work_item_id, conversation_id, organization_id, project_id, created_at, closed_at)
VALUES (?, ?, ?, ?, ?, NULL)`, record.WorkItemID, record.ConversationID, record.OrganizationID, record.ProjectID, conversationTime(record.CreatedAt)); err != nil {
		return fmt.Errorf("insert coordinator item: %w", err)
	}
	return nil
}

// closeCoordinatorItem marks the item closed. Closing an already closed item
// keeps the first closure: the row records when the conversation stopped
// expecting that attempt, not the last write.
func (s *conversationStore) closeCoordinatorItem(ctx context.Context, tx *sql.Tx, workItemID string, now time.Time) error {
	if _, err := tx.ExecContext(ctx, "UPDATE coordinator_items SET closed_at = ? WHERE work_item_id = ? AND closed_at IS NULL", conversationTime(now), workItemID); err != nil {
		return fmt.Errorf("close coordinator item: %w", err)
	}
	return nil
}

// readConversationByCoordinatorItem resolves the conversation an open
// coordinator item answers. A closed item resolves nothing: its attempt is
// over and the next message opens a new item.
func (s *conversationStore) readConversationByCoordinatorItem(ctx context.Context, query nativeQueryer, organization tracker.OrganizationID, project tracker.ProjectID, workItemID string) (conversationRecord, error) {
	record, err := scanConversation(query.QueryRowContext(ctx, "SELECT "+conversationReadColumns+` FROM conversations
WHERE conversations.id = (SELECT conversation_id FROM coordinator_items WHERE work_item_id = ? AND organization_id = ? AND project_id = ? AND closed_at IS NULL)
 AND conversations.organization_id = ? AND conversations.project_id = ?`, workItemID, organization, project, organization, project))
	if errors.Is(err, sql.ErrNoRows) {
		return record, nativeNotFound()
	}
	if err != nil {
		return record, fmt.Errorf("read coordinator conversation: %w", err)
	}
	return record, nil
}
