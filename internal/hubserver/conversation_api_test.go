...
func TestConversationAPICreate(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			wantExecution := conversation.ExecutionIdle
			if tt.receipt {
				// A first message dispatches a coordinator turn to a runner.
				wantExecution = conversation.ExecutionWaitingForRunner
			}
			if record.Owner.PrincipalID != f.ownerID || record.ProjectID != f.project.ID || record.Execution.Status != wantExecution {
			...
			if created.Receipt.Key != "first" || created.Receipt.Status != conversation.DeliveryQueued || created.Receipt.Error != nil || created.Receipt.MessageID == "" {
				t.Fatalf("receipt = %#v", created.Receipt)
			}
			snapshot := f.snapshot(t, f.token, record.ID)
			if len(snapshot.Messages) != 1 || snapshot.Messages[0].Kind != conversation.MessageText || snapshot.Messages[0].Role != conversation.RoleUser || snapshot.Messages[0].Delivery != conversation.DeliveryQueued {
...
func TestConversationAPICommands(t *testing.T) {
	...
	t.Run("message replay and conflict", func(t *testing.T) {
		...
		if receipt.Status != conversation.DeliveryQueued || receipt.Error != nil || receipt.MessageID == "" {
...
func TestConversationAPILink(t *testing.T) {
	...
	t.Run("links and shares", func(t *testing.T) {
		...
		// The chat's first message opened a coordinator item as issue #1.
		if linked.Issue.ID != string(linked.Issue.WorkItemID) || linked.Issue.Identifier != f.project.Name+"#2" {
	...
	t.Run("second link conflicts", func(t *testing.T) {
		...
		var issues int
		if err := f.service.database.db.QueryRowContext(t.Context(), `SELECT count(*) FROM issues i WHERE i.organization_id = ? AND i.project_id = ? AND `+notCoordinatorItemClause,
