...
// watchPersonAuthority is section 18.2's 30-second periodic re-check: the
// fallback for anything the authority.changed event misses. The event path is
// revokePrincipal; this is what catches a change nobody thought to publish.
...
func (s *Service) openWorkspaceWorkerRelay(c echo.Context) error {
	...
	if previous != nil {
		...
		// The processes of any in-flight runs were on the connection that just
		// lost the workspace, so nothing will ever report their exit. The new
		// connection has no knowledge of them either: a run it was not asked
		// for is a run it is not serving.
...
// finishExecRun writes a run's terminal status, its accumulated output and the
// action_run.<status> event, and wakes the project's subscribers after the
// commit.
//
