...
// revokePrincipal closes every person connection of one principal, anywhere on
// the hub. It is the authority.changed path of section 18.2: membership, role,
// grant, session and setting mutations call it, and the relay closes every
// affected connection with revoked.
func (r *workspaceRelay) revokePrincipal(principalID string) {
	if principalID == "" {
		return
	}
	r.mu.Lock()
	...
	for _, room := range r.rooms {
		for _, person := range room.people {
			if person.principalID == principalID {
				connections = append(connections, person)
			}
		}
	}
...
// revokeOrganization closes every person connection in the hub. A hosted hub
// serves one organization, so an organization-wide authority change affects
// everyone; the argument is kept so the call site reads as section 18.2's
// authority.changed {principal_id | organization_id}.
func (r *workspaceRelay) revokeOrganization(organizationID string) {
	if organizationID == "" {
		return
	}
	r.mu.Lock()
...
// authorityChanged is section 18.2's authority.changed, called from every
// mutation that can change what a person may do: membership, role, grant,
// session and setting changes. The relay closes every affected connection with
// revoked rather than letting a socket outlive the grant that opened it.
//
// It takes the hosted subject because that is what the member endpoints work
// in; the relay knows connections by principal, so the two are joined here
// rather than at four call sites.
func (s *Service) authorityChanged(ctx context.Context, userID string) {
	if s.workspaces == nil || s.workspaces.relay == nil || s.config.Hosted == nil || strings.TrimSpace(userID) == "" {
		return
	}
	var principal string
	err := s.database.db.QueryRowContext(ctx, "SELECT principal_id FROM hosted_members WHERE user_id = ?", userID).Scan(&principal)
	if err != nil {
		if !errors.Is(err, sql.ErrNoRows) {
			s.config.Logger.Warn("workspace.authority_change_unresolved", "user_id", userID, "error", err)
		}
		// A member the hub no longer knows has no principal to revoke by, so
		// the organization-wide sweep is the only correct answer: a removed
		// member must not keep a socket open because their own row went first.
		s.workspaces.relay.revokeOrganization(s.config.Hosted.OrganizationID)
		return
	}
	s.workspaces.relay.revokePrincipal(principal)
}
