...
func (c *conversationService) acceptCommand(ctx context.Context, tx *sql.Tx, scope nativeScope, record *conversationRecord, command conversation.Command, now time.Time) (conversation.Receipt, error) {
	...
	case conversation.CommandMessage:
		...
		coordinatorItem := false
		switch {
		case !linked && c.coordinator.Available():
			// The hub-side coordinator reads saved messages from the store,
			// so its backlog is bounded by the same queue as a worker's.
			if err := c.requireQueueCapacity(ctx, tx, record.ID, command.Kind); err != nil {
				return receipt, err
			}
			message.Delivery = conversation.DeliverySaved
		case !linked:
			// No hub backend: the turn is dispatched to a customer runner
			// through a coordinator work item (decisions section 9.1).
			if err := c.requireQueueCapacity(ctx, tx, record.ID, command.Kind); err != nil {
				return receipt, err
			}
			if _, err := c.ensureCoordinatorItem(ctx, tx, scope, *record, now); err != nil {
				return receipt, err
			}
			message.Delivery = conversation.DeliveryQueued
			coordinatorItem = true
		default:
		...
		if coordinatorItem {
			if execution, waiting := coordinatorItemExecution(*record); waiting {
	...
	case conversation.CommandCancel:
		...
		// A runner-dispatched coordinator turn is stopped the same way a
		// linked attempt is: the client-facing kind stays cancel while the
		// control the runner receives is an interrupt (section 9.1).
		bound, err := c.coordinatorAttemptBound(ctx, tx, *record)
		if err != nil {
			return receipt, err
		}
		if !bound {
...
func (c *conversationService) retryMessage(ctx context.Context, tx *sql.Tx, scope nativeScope, record *conversationRecord, command conversation.Command, now time.Time) (conversationMessageRecord, error) {
	...
	coordinatorItem := false
	switch {
	case record.WorkItemID == "" && c.coordinator.Available():
		// The hub-side coordinator reads saved user messages itself.
...
func (s *Service) linkConversation(c echo.Context) error {
	...
	err = s.nativeMutation(c, tracker.Mutation{IdempotencyKey: request.Key}, request, func(ctx context.Context, tx *sql.Tx, scope nativeScope, now time.Time) (any, error) {
		...
		// A handoff replaces the coordinator turn: its item stops being the
		// conversation's open item and the linked attempt takes over.
		if item, open, err := service.store.openCoordinatorItem(ctx, tx, record.ID); err != nil {
			return nil, err
		} else if open {
			if err := service.closeCoordinatorItem(ctx, tx, scope, item, now); err != nil {
				return nil, err
			}
		}
