...
// exist now (section 18.1), but none of them produces a diff: the live
// workspace diff channel of 18.5 is not built, so the attempt-producer path is
// still the only one implemented and every other producer is refused with
// stale_execution rather than guessed at.
//
