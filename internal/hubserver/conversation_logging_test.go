...
func TestConversationLoggingCommandPath(t *testing.T) {
	t.Parallel()
	// The first message already queues a coordinator control, so the queue
	// holds one before the link.
	...
	t.Run("receipt", func(t *testing.T) {
		// No hub coordinator is configured, so the first message opens a
		// coordinator work item and waits for a runner.
