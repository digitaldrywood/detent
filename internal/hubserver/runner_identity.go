...
func runnerOperationAllowed(c echo.Context, operations []string) bool {
	...
	case strings.HasPrefix(path, nativeBase+"/workspaces/:workspace/worker/"):
		...
		operation = runnerauth.Claim
	case path == nativeBase+"/work-items/:item/events", path == nativeBase+"/conversations/:conversation/turn-events",
		// A stored attempt diff accompanies the run event that references it
		// (decisions section 18.5), so it needs the same operation the event
		// needs and nothing wider.
		path == nativeBase+"/attempts/:attempt/diff":
