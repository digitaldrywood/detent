...
func TestHostedWorkAccess(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			// The client route is the same shell for everybody; the data
			// behind it is what access decides (decisions section 12).
			shell := f.request(t, user, http.MethodGet, "/work/items/"+string(item), nil)
			if strings.Contains(shell.Body.String(), "private-") {
				t.Error("application shell carried project content")
			}
			for _, path := range []string{f.base + "/work-items/" + string(item), api + "/changes/" + change.ID} {
				...
				if test.allowed {
					...
					if response.Code == http.StatusOK {
						t.Errorf("revoked read allowed: %s", path)
					}
					if strings.Contains(response.Body.String(), "private-") {
						t.Error("denial exposed private content")
					}
				}
				if !strings.Contains(response.Header().Get("Content-Type"), "application/json") || response.Header().Get("Cache-Control") != "no-store" {
					t.Errorf("hosted work must return uncached JSON: %s", path)
				}
			}
...
// TestHostedWorkMissingAndForeignResources keeps the opaque-404 rule the
// removed pages carried: an unknown or foreign record never confirms itself.
func TestHostedWorkMissingAndForeignResources(t *testing.T) {
	...
	base := browserHostedOrganizationBase + "/projects/"
	for _, path := range []string{
		base + f.project + "/work-items/missing",
		base + f.privateProject + "/work-items/" + item,
		base + f.project + "/work-items/" + item + "/changes/missing",
	} {
		t.Run(path, func(t *testing.T) {
			response := f.api(t, "owner", http.MethodGet, path, nil, http.StatusNotFound)
