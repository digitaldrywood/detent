...
type Service struct {
	...
	// clientBuild identifies the client bundle this process serves. The
	// embedded filesystem cannot change while the process runs, so it is
	// hashed once here rather than on every bootstrap or version poll.
