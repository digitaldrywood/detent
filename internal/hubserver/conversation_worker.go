...
type conversationBindResponse struct {
	...
	// Coordinator reports that the work item is a coordinator item rather
	// than a linked issue: the run answers the conversation and produces no
	// worktree, branch or pull request (decisions section 9.3).
...
func (r *conversationWorkerRouter) bind(e echo.Context) error {
	...
	err := c.transact(ctx, func(tx *sql.Tx, now time.Time) error {
		...
		record, coordinator, err = c.readBoundConversation(ctx, tx, scope, item)
		if err != nil {
			return err
		}
		...
		if thread == "" {
			...
			record.ProviderThreadOrigin = conversation.ThreadOriginFor(coordinator)
		}
		...
		resume, err := c.resumeFor(ctx, tx, record, scope, thread, named, coordinator, pending)
		if err != nil {
			return err
		}
...
func (r *conversationWorkerRouter) unbind(e echo.Context) error {
	...
	err := c.transact(ctx, func(tx *sql.Tx, now time.Time) error {
		...
		if coordinator {
			// The coordinator item answered its turn: it moves to a terminal
			// state and stops being the conversation's open item, so the
			// next message dispatches a new one (decisions section 9.1).
			// The item is the one this unbind names, never a newer one.
			if err := c.closeCoordinatorItem(ctx, tx, scope, coordinatorItemRecord{WorkItemID: item, ConversationID: record.ID}, now); err != nil {
				return err
			}
		}
		changed = true
...
func (c *conversationService) pollControls(ctx context.Context, scope nativeScope, id string, identity conversationWorkerIdentity, after int64) (conversationControlsResponse, error) {
	...
	err := c.transact(ctx, func(tx *sql.Tx, now time.Time) error {
		var item string
		var err error
		record, item, _, err = c.loadWorkerConversation(ctx, tx, scope, id)
		if err != nil {
			return err
		}
...
type conversationTurn struct {
	...
	execution conversation.Execution
	// coordinator reports that the bound work item is a coordinator item, so
	// a thread this turn opens is recorded with the coordinator origin
	// (decisions section 9.3).
	coordinator bool
	changed     bool
	now         time.Time
}
...
func (r *conversationWorkerRouter) turnEvents(e echo.Context) error {
	...
	err := c.transact(ctx, func(tx *sql.Tx, now time.Time) error {
		...
		var coordinator bool
		var err error
		record, item, coordinator, err = c.loadWorkerConversation(ctx, tx, scope, id)
		if err != nil {
			return err
		}
		...
		turn := &conversationTurn{scope: scope, record: &record, execution: record.Execution, coordinator: coordinator, now: now}
...
func (c *conversationService) applyTurnStarted(ctx context.Context, tx *sql.Tx, turn *conversationTurn, event conversationTurnEvent) error {
	...
	if thread := strings.TrimSpace(event.ThreadID); thread != "" {
		...
		turn.record.ProviderThreadOrigin = conversation.ThreadOriginFor(turn.coordinator)
	}
