...
func TestConversationStreamReplaysAndFollows(t *testing.T) {
	...
	// The first message opens a coordinator work item, so the conversation
	// and its execution move to waiting_for_runner before the receipt.
	...
	if first.ID != "1" || first.Event != string(conversation.EventMessageAccepted) ||
		second.ID != "2" || second.Event != string(conversation.EventConversationUpdated) ||
		third.ID != "3" || third.Event != string(conversation.EventExecutionUpdated) ||
		fourth.ID != "4" || fourth.Event != string(conversation.EventCommandReceipt) {
		t.Fatalf("frames = %#v, %#v, %#v, %#v", first, second, third, fourth)
	}
	...
	if heartbeat.Event != string(conversation.EventHeartbeat) || heartbeat.ID != "" || heartbeat.Data != `{"seq":4}` {
		t.Fatalf("heartbeat = %#v", heartbeat)
	}
	// The second message reuses the open coordinator item, so only the
	// message and its receipt reach the stream.
	response := f.command(t, f.token, id, conversation.Command{Key: "second", Kind: conversation.CommandMessage, Text: "Follow up"})
	requireNativeStatus(t, response, http.StatusOK)
	fifth := stream.nextEvent(t)
	sixth := stream.nextEvent(t)
	if fifth.ID != "5" || fifth.Event != string(conversation.EventMessageAccepted) || sixth.ID != "6" || sixth.Event != string(conversation.EventCommandReceipt) {
		t.Fatalf("frames = %#v, %#v", fifth, sixth)
	}
	stream.close()

	reconnect := f.open(t, f.token, id, 6)
	...
	partial := f.open(t, f.token, id, 4)
	if frame := partial.nextEvent(t); frame.ID != "5" {
