...
// requireUnreservedLabels refuses the pull request action label on issue
// content that did not come from the hub. The label selects a merge-lane item
// the runner executes without producing a deliverable (decisions section
// 18.6), so a tracker writer who could set it could queue an action the pull
// request panel never accepted.
func requireUnreservedLabels(ctx context.Context, labels []string) error {
	if pullRequestActionLabelAllowed(ctx) {
		return nil
	}
	for _, label := range labels {
		if strings.EqualFold(strings.TrimSpace(label), pullRequestActionLabel) {
