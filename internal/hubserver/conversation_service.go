...
type ConversationConfig struct {
	// Enabled mounts the conversation API, streams and the client shell.
	Enabled bool
	...
	// when Backend is set.
	Workspace string
	// Model and ReasoningEffort are passed to coordinator turns.
	Model           string
...
func (c ConversationConfig) normalized() ConversationConfig {
	...
	if c.QuestionTimeout <= 0 {
		c.QuestionTimeout = runner.DefaultConversationQuestionTimeout
	}
	if c.SettleWindow <= 0 {
...
// durable store is the queue: Wake only signals that queued user messages
...
WHERE m.role = 'user' AND m.delivery IN ('saved', 'queued')`)
	...
	c.logger.Info("conversation.wake_pending", "coordinator", coordinator, "controls", controls)
	return nil
}
