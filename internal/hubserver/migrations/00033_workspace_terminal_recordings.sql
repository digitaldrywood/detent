...
-- shells leaves three.
--
-- The bytes live in the hub rather than in the artifact service, on the
-- precedent migration 00030 set for an action run's output and 00026 set for a
-- stored diff: a bounded, audience-scoped text blob the hub keeps, where the cap
-- is what makes keeping it safe. The audience is the part that differs, and it
-- is why this is a table of its own rather than a column on the relay session
-- row. An action run's output has exactly the issue's read audience; a terminal
-- recording's is narrower than the issue's -- "the person who ran the session
