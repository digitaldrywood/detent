...
--
-- attempt_id carries no foreign key: a workspace may be opened on an attempt the recorder has not ordered yet, and
...
CREATE TABLE workspace_sessions (
  ...
  -- machine_hostname and worktree_path are where the worktree actually is
  -- (decisions section 18.13), reported on the runner's heartbeat so a client
  -- can tell whether an editor link on this machine would reach it.
  -- machine_hostname is not machine_id: the id is the hub's own binding
  -- identifier, the hostname is what a person recognises.
...
--
-- A workspace is its own work item kind, detent:workspace: it may check out the
-- repository, it does not end when a turn ends, and it stays claimed until the
-- workspace closes. The claim gate and the dispatcher both resolve the workspace from the issue the runner was
