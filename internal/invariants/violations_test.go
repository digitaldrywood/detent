...
func TestSourceViolations(t *testing.T) {
	for _, tt := range []struct{ name, file, body, want string }{
		...
		{"merge fairness exclusion", "internal/orchestrator/new.go", `func mergeFairnessBlocks() {}`, "INV-10"},
		{"merge fairness reason", "internal/scheduler/new.go", `const reason = "merge_fairness_head_reserved"`, "INV-10"},
		{"historical merge fairness", "internal/config/historical_decision_reasons.go", `const reason = "merge_fairness_head_reserved"`, ""},
