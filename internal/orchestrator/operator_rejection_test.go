	...
	for _, tt := range []struct {
		...
		for _, completion := range []bool{false, true} {
			...
			t.Run(name, func(t *testing.T) {
				...
				cfg := normalizeConfig(Config{ActiveStates: []string{"Rework"}, AutoPromote: AutoPromoteConfig{Enabled: true, PassState: "Human Review", GateWaitState: "review", Gate: gate.Config{Kind: gate.KindCommand, RequireAutomatedReview: new(false)}}})
				...
				backend, _ := openLaneMutationTestStore(t, t.Context(), cfg.Project.ID, issue, now)
				thin := cloneIssue(issue)
				thin.PullRequest = nil
				tracker := &liveReworkConnector{autoPromoteTickConnector: &autoPromoteTickConnector{stateIssues: []connector.Issue{thin}}, live: issue}
