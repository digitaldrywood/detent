...
func (o *Orchestrator) delegateNativeMergeQueueIssues(
	...
	for _, candidate := range staleMergingQueueIssues(out, o.cfg, state, now) {
		...
		if requiresAutomatedReview(o.cfg) && candidate.PullRequest.AutomatedReviewPending() {
...
func nativeMergeQueueCandidate(issue connector.Issue, cfg Config) bool {
	...
	if len(pullRequest.UnresolvedReviewThreads) > 0 || (requiresAutomatedReview(cfg) && pullRequest.AutomatedReviewPending()) {
		return false
	}
