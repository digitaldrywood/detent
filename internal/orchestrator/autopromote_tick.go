...
func staleMergingQueueIssues(issues []connector.Issue, cfg Config, state *State, now time.Time) []connector.Issue {
	...
	prioritizeReadyMergingIssues(queue, state, now, cfg)
	return queue
}
...
func (o *Orchestrator) mergeWorkerDispatchCandidates(state *State, issues []connector.Issue, now time.Time) []connector.Issue {
	...
	for _, issue := range candidates {
		...
		o.logMergeWorkerPickup(issue, "stale_merging",
			...
			"selection_reason", mergeWorkerSelectionReason(issue, state, now, o.cfg),
...
func prioritizeReadyMergingIssues(issues []connector.Issue, state *State, now time.Time, cfg Config) mergingIssuePriority {
	fairnessAge := cfg.MergeFairnessAge
...
func mergeWorkerSelectionReason(issue connector.Issue, state *State, now time.Time, cfg Config) string {
	fairnessAge := cfg.MergeFairnessAge
