...
func TestLocalGitCleanupPreservesUncommittedAndUnpushedWork(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{name: "uncommitted work", change: "dirty", wantBlocked: true},
		{name: "unpushed commit", change: "unpushed", wantBlocked: true},
		{name: "pushed commit", change: "pushed"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			...
			identifier := "DD-CLEANUP-" + strings.ToUpper(tt.change)
			if tt.change == "" {
				identifier = "DD-CLEANUP-CLEAN"
			}
			info, err := backend.Create(t.Context(), Issue{Identifier: identifier})
			if err != nil {
				t.Fatalf("Create() error = %v", err)
			}
			...
			case "unpushed", "pushed":
				...
				runGit(t, info.Path, "commit", "-m", "test: preserve cleanup work")
				if tt.change == "pushed" {
					runGit(t, info.Path, "push", "-u", "origin", "HEAD:"+info.Branch)
				}
			}

			err = backend.Cleanup(t.Context(), identifier)
			if tt.wantBlocked {
				if !errors.Is(err, ErrWorkspaceHasUnpreservedWork) {
					t.Fatalf("Cleanup() error = %v, want ErrWorkspaceHasUnpreservedWork", err)
				}
				if _, statErr := os.Stat(info.Path); statErr != nil {
					t.Fatalf("preserved workspace stat error = %v", statErr)
				}
				if !branchExists(t, source, info.Branch) {
					t.Fatalf("branch %q was deleted", info.Branch)
				}
				return
			}
			if err != nil {
			...
			if _, statErr := os.Stat(info.Path); !errors.Is(statErr, os.ErrNotExist) {
				t.Fatalf("workspace exists after cleanup, stat error = %v", statErr)
			}
			if branchExists(t, source, info.Branch) {
				t.Fatalf("branch %q still exists after cleanup", info.Branch)
			}
		})
	}
}

func TestLocalGitCleanupPreservesUnpushedBranchWhenWorkspaceIsMissing(t *testing.T) {
	t.Parallel()
	...
	identifier := "DD-MISSING-UNPUSHED"
	info, err := backend.Create(t.Context(), Issue{Identifier: identifier})
	if err != nil {
		t.Fatalf("Create() error = %v", err)
	}
	...
	runGit(t, info.Path, "add", "committed.txt")
	runGit(t, info.Path, "commit", "-m", "test: preserve missing workspace branch")
	runGit(t, source, "worktree", "remove", "--force", info.Path)
