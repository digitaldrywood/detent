...
func TestAttachLabelIssuePullRequestReferencesReusesUnchangedIssues(t *testing.T) {
	...
	server := newGraphQLTestServer(t, []graphqlTestResponse{
		...
		{body: `{"data":{"nodes":[{"__typename":"Issue","id":"I_1","number":1,"repository":{"nameWithOwner":"example/repo"},"timelineItems":{"nodes":[{"__typename":"LabeledEvent","createdAt":"2026-09-02T10:00:00Z","label":{"name":"detent:human-review"}}]},"closedByPullRequestsReferences":{"nodes":[]}}]}}`},
	...
	c := newGitHubTestConnector(t, server, Config{GitHubStatusSource: GitHubStatusSourceLabel, Repository: "example/repo"})
	...
	tests := []struct {
		...
		{name: "lane changed", firstState: "Human Review", firstAt: laneRevision, secondAt: firstRevision, wantIDs: []string{"I_1"}, wantStage: time.Date(2026, 9, 2, 10, 0, 0, 0, time.UTC)},
		{name: "PR linked", firstState: "Human Review", firstAt: laneRevision, secondAt: prRevision, wantIDs: []string{"I_2"}, wantStage: time.Date(2026, 9, 2, 10, 0, 0, 0, time.UTC), wantPR: 20},
		{name: "linked PR unchanged", firstState: "Human Review", firstAt: laneRevision, secondAt: prRevision, wantStage: time.Date(2026, 9, 2, 10, 0, 0, 0, time.UTC), wantPR: 20},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func TestConnectorFetchLabelIssuesByStatesAttachesCurrentAgentBranchPullRequest(t *testing.T) {
	...
	server := newGraphQLTestServer(t, []graphqlTestResponse{
		...
		},
		{
			...
			path:   "/",
			body:   `{"data":{"repository":{"pullRequest":{"number":434,"url":"https://github.com/digitaldrywood/digitaldrywood/pull/434","state":"OPEN","headRefName":"detent/digitaldrywood-digitaldrywood_digitaldrywood_433-a212db2634a4","headSHA":"sha-434","activityAt":null,"commits":{"nodes":[{"commit":{"oid":"sha-434","committedDate":"2026-06-05T10:30:00Z"}}]}}}}}`,
		},
		{
			method: http.MethodGet,
...
func TestConnectorFetchIssuesByStatesScopesPullRequestHydrationByIssueState(t *testing.T) {
	...
	server := newGraphQLTestServer(t, []graphqlTestResponse{
		...
		{
			method: http.MethodPost, path: "/", body: `{"data":{"repository":{"pullRequest":{"number":120,"url":"https://github.com/digitaldrywood/detent/pull/120","state":"OPEN","headRefName":"detent/digitaldrywood_detent_20","headSHA":"sha-120","activityAt":null,"mergedAt":null}}}}`,
		},
		{
			method: http.MethodGet,
...
func TestConnectorUpdateIssueStateReplacesStatusLabels(t *testing.T) {
	...
	for _, selector := range []string{"detent:host-a", "DETENT:HOST-A"} {
		t.Run(selector, func(t *testing.T) {
			...
			server := newGraphQLTestServer(t, []graphqlTestResponse{
				...
				},
				{
					...
					body:   `{"node_id":"I_485","number":485,"title":"Installer packages","body":"","state":"open","html_url":"https://github.com/digitaldrywood/detent/issues/485","assignees":[],"labels":[{"name":"` + selector + `"},{"name":"detent:todo"},{"name":"bug"}]}`,
				},
				{
					method: http.MethodGet,
					path:   "/repos/digitaldrywood/detent/issues/485",
				...
				},
				{
					...
					body:   `[{"name":"` + selector + `"},{"name":"bug"},{"name":"detent:in-progress"}]`,
				},
			})
			c := newGitHubTestConnector(t, server, Config{
				GitHubStatusSource: GitHubStatusSourceLabel,
			...
			if !reflect.DeepEqual(labels, []any{selector, "bug", "detent:in-progress"}) {
				t.Fatalf("labels body = %#v, want selector, bug, and detent:in-progress", labels)
			}
		})
	}
}
		...
		t.Run(selector, func(t *testing.T) {
			...
			server := newGraphQLTestServer(t, []graphqlTestResponse{
				...
				},
				{
					method: http.MethodGet,
					path:   "/repos/digitaldrywood/detent/issues/485",
					body:   `{"node_id":"I_485","number":485,"title":"Installer packages","body":"","state":"open","html_url":"https://github.com/digitaldrywood/detent/issues/485","assignees":[],"labels":[{"name":"` + selector + `"},{"name":"detent:todo"},{"name":"detent:in-progress"},{"name":"bug"},{"name":"stage:s6"}]}`,
				},
				{
					method: http.MethodPut,
					path:   "/repos/digitaldrywood/detent/issues/485/labels",
					body:   `[{"name":"` + selector + `"},{"name":"bug"},{"name":"stage:s6"}]`,
				},
			})
			c := newGitHubTestConnector(t, server, Config{
				GitHubStatusSource: GitHubStatusSourceLabel,
			...
			labels := body["labels"].([]any)
			if !reflect.DeepEqual(labels, []any{selector, "bug", "stage:s6"}) {
...
func TestConnectorFetchCandidateIssuesSurfacesStatusLabelConflict(t *testing.T) {
	...
	server := newGraphQLTestServer(t, []graphqlTestResponse{
		...
		{
			method: http.MethodPost, path: "/", body: `{"data":{"repository":{"pullRequest":{"number":609,"url":"https://github.com/digitaldrywood/detent/pull/609","state":"OPEN","headRefName":"detent/detent-digitaldrywood_detent_606-97bf7548e359","headSHA":"sha-609","activityAt":null,"mergedAt":null}}}}`,
		},
		{
			method: http.MethodGet,
