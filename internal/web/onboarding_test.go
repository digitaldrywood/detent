	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, want := range append(tt.want,
				...
				"Whenever you touch CI\nconfiguration or perform a review",
				"Require passing PR-head checks when jobs run\nthere",
				"For merge-group-only CI, report expected PR skips and require passing\nmerge-group checks before merge",
				"detent doctor to infer\nrequired stages or inspect CI",
				"```detent-status\nschema: 1\nstatus: in_progress\nblockers: []\nhuman_action: null\n```",
