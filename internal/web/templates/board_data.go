...
func boardExceptions(data DashboardData, boardActions bool) []primitives.Exception {
	...
	for _, row := range data.Snapshot.Blocked {
		if BlockedRecoveryWaiting(row.NeedsHumanAttention, row.Source, row.RecoveryAction, row.RecoveryReason, row.Error) {
			continue
		}
...
func boardExceptionDetail(row telemetry.Blocked, now time.Time) string {
	...
	if detail == "" && BlockedRecoveryWaiting(row.NeedsHumanAttention, row.Source, row.RecoveryAction, row.RecoveryReason, row.Error) {
...
func boardCardExtra(card projectKanbanCard, view boardCardView) (primitives.Kind, string, bool) {
	...
	if BlockedRecoveryWaiting(card.BlockedHumanAttention, card.BlockedSource, card.BlockedRecoveryAction, card.BlockedRecoveryReason, card.BlockedReason) {
...
func BlockedRecoveryWaiting(needsHumanAttention bool, source telemetry.BlockedSource, recoveryAction string, recoveryReason string, reason string) bool {
...
func boardCardNumberClass(card boardCardView) string {
	if card.Done {
		return "flex-none max-w-full break-all text-sec"
	}
	return "flex-none max-w-full break-all text-text"
}
