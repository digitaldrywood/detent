...
func resolveRuntimePort(ctx context.Context, input runtimeInput, deps runtimeDeps) (RuntimeIntValue, error) {
	if input.Flags.Port.Set {
		if input.Flags.Port.Value < 0 {
			return RuntimeIntValue{}, ValidationError("port must be greater than or equal to 0")
		}
	...
	if raw := strings.TrimSpace(deps.lookupEnv("PORT")); raw != "" {
		...
		if err != nil || port < 0 {
			return RuntimeIntValue{}, ValidationError("PORT must be an integer greater than or equal to 0")
		}
...
func resolveRuntimeGitHubToken(ctx context.Context, cfg *globalconfig.Config, deps runtimeDeps) (RuntimeSecret, []RuntimeWarning, error) {
	...
	if requiresRuntimeToken {
		return RuntimeSecret{}, nil, GitHubAuthError(fmt.Errorf("GITHUB_TOKEN is not set, github_token is not configured, and no usable tracker.api_key was found. %s", githubAuthHint))
	}
	return RuntimeSecret{Required: false}, nil, nil
}
...
func resolveConfiguredGitHubToken(ctx context.Context, token string, deps runtimeDeps) (RuntimeSecret, error) {
	if githubTokenSentinel(token) {
		...
		if err != nil {
			return RuntimeSecret{}, GitHubAuthError(fmt.Errorf("resolve github_token via gh auth token: %w. %s", err, githubAuthHint))
		}
		resolved = strings.TrimSpace(resolved)
		if resolved == "" {
			return RuntimeSecret{}, GitHubAuthError(fmt.Errorf("resolve github_token via gh auth token: empty token. %s", githubAuthHint))
		}
