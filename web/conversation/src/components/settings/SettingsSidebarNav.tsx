...
import {
  ...
  useSettingsNavItems,
} from "./useAvailableSettingsSearchItems";

type SectionIcons = Readonly<Record<string, ComponentType<{ className?: string }>>>;

function SettingsSectionIcon({ to, icons }: { to: SettingsPath; icons: SectionIcons }) {
  const Icon = icons[to];
  if (!Icon) return null;
...
export function SettingsSidebarNav({ pathname }: { pathname: string }) {
  ...
  const sectionIcons = useMemo<SectionIcons>(
    () => Object.fromEntries(navItems.map((item) => [item.to, item.icon])),
    [navItems],
  );
          ...
          {isSearching ? (
            ...
            >
              {results.map((item, index) => (
                <SidebarMenuItem key={item.id} role="presentation">
                  ...
                  >
                    <SettingsSectionIcon to={item.to} icons={sectionIcons} />
            ...
            <SidebarMenu className="ps-px">
              {navItems.map((item) => {
                ...
                const isActive =
                  !disabled && (pathname === item.to || pathname.startsWith(`${item.to}/`));
                const row = (
                  <SidebarMenuButton
                    isActive={isActive}
                    ...
                    onClick={() => {
                      if (!disabled) handleSectionClick(item.to);
                    }}
                  >
                    <Icon />
                  ...
                  <SidebarMenuItem key={item.to}>
                    {disabled && item.reason !== undefined ? (
                      <Tooltip>
                        ...
                        <TooltipPopup side="right" className="max-w-64">
                          {item.reason}
                        </TooltipPopup>
                      </Tooltip>
                    ) : (
