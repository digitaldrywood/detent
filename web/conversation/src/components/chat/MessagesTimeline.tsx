...
// Seam (1 of 1): see `renderItem` below.
import { DetentTimelineRow, detentRowTestId } from "~/app/components/TimelineCards";
  ...
  const renderItem = useCallback(
    ({ item }: { item: MessagesTimelineRow }) => (
      <div
        ...
        {...detentRowTestId(item)}
      >

        <DetentTimelineRow messageId={item.kind === "message" ? item.message.id : null}>
...
function WorkEntryIcon({ name, className }: { name: WorkEntryIconName; className: string }) {
  switch (name) {
    ...
    case "t3-code":
      return <DetentWordmark className={className} aria-hidden />;
    case "check":
