...
export default function Sidebar() {
  ...
  const openProjectSettings = useCallback(
    (projectGroup: SidebarProjectSnapshot) => {
      ...
      void router.navigate({
        to: "/chat/p/$projectId",
        params: { projectId: projectGroup.projectKey },
      });
    },
  ...
  return (
    // Seam (1 of 2). `SidebarLandmark` is the `aside` that names this column
    // for a browser reader; `SidebarDestinations` below is Detent's Work and
    // Browse rows. Both live in `app/adapters/sidebarDestinations.tsx` and are
    // the only Detent content in this file — everything else is upstream's.
    <SidebarLandmark>
      ...
      >
        <SidebarGroup className="ps-[calc(var(--sidebar-content-inset)+1px)] pe-[var(--sidebar-content-inset)] pb-1 pt-0">
          {/* Seam (2 of 2): Detent's cross-cutting destinations, above the
              thread list (decisions.md §17.4). */}
          <SidebarDestinations />
          {isSearchingThreads ? (
