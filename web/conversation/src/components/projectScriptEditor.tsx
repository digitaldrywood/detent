...
/**
 * §18.7, quoted: the sentence the disabled preview switch carries. It is the
 * exact string decisions.md fixes, so a grep for it finds the rule and not a
 * paraphrase of it.
 */
export const PREVIEW_UNAVAILABLE_REASON = "Browser preview is not available on Detent Cloud yet.";
...
export function ProjectScriptEditorDialog({
  ...
  // The §18.7 reason's own id, so the disabled switch can name it (Detent).
  const previewReasonId = React.useId();
        ...
        <DialogPopup>
          ...
          <DialogPanel>
            <form id={formId} className="space-y-4" onSubmit={submit}>
              ...
              {/* §18.7, §16: upstream disables this switch while the URL is
                  empty; Detent disables it always and says why, because there
                  is no live preview for an action to open yet. Their row
                  markup and their own `opacity-60` disabled treatment are
                  kept; the reason arrives through the `Tooltip` this client
                  already uses for a disabled control (`RightPanelTabs.tsx`'s
                  `DisabledReasonTooltip`) rather than a bare `title`, so it
                  reaches a keyboard reader too. `data-disabled:pointer-events
                  -auto` is what lets a disabled switch still be hovered. */}
              <Tooltip>
                <TooltipTrigger
                  render={
                    <label className="flex items-center justify-between gap-3 rounded-md border border-border/70 px-3 py-2 text-sm opacity-60 dark:border-transparent dark:bg-white/[0.035]" />
                  }
                >
                  <span>
                    Open preview automatically when this action runs
                    {/* The tooltip only exists in the DOM while it is open, so
                        the reason is also here for a reader who never hovers
                        anything. One sentence, two routes to it. */}
                    <span className="dc-sr-only" id={previewReasonId}>
                ...
                <TooltipPopup side="top">{PREVIEW_UNAVAILABLE_REASON}</TooltipPopup>
              </Tooltip>
