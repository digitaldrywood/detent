...
import type { DailyTotals, HourlyTotals } from "./adapter.ts";
import {
  formatDayShort,
  formatHourShort,
...
import { PROVIDER_ORDER, presentationFor } from "./usageProviders.ts";
          ...
          >
            ...
            {paths.map(({ provider, area }) => (
              <path
                key={provider}
                d={area}
                fill={presentationFor(provider).color}
                fillOpacity={0.12}
              />
            ))}
            {paths.map(({ provider, line }) => (
              <path
                ...
                d={line}
                fill="none"
                stroke={presentationFor(provider).color}
                strokeWidth={2}
                vectorEffect="non-scaling-stroke"
              />
            ))}
          ...
          {hoveredPeriod === undefined ? null : (
            ...
            >
              ...
              {providers.map((provider) => {
                const { label, mark: Mark } = presentationFor(provider);
                return (
